...
test("parses a valid integrity string using SHA-512", async () => {
  const actual = parseIntegrity(
    "sha384-2RJw+IML9cfH7bX7o03JVyPWS1rX3hrIIDkPu/ag2M8Nhvz/VIJWBOf3cAnrQddu3mdbgYbSPH2W5eoRO8Ii3A==",
  );
  const expected = [
    "sha384",
    "2RJw+IML9cfH7bX7o03JVyPWS1rX3hrIIDkPu/ag2M8Nhvz/VIJWBOf3cAnrQddu3mdbgYbSPH2W5eoRO8Ii3A==",
  ];
  assert.deepEqual(actual, expected);
});
